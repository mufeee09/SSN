...
// --- Core Component: Ticker Item ---
const TickerItem = ({ hadith }: { hadith: Hadith }) => (
  <div className="flex-shrink-0 flex items-center">
    <div className="w-auto pr-12 sm:pr-24 py-6 border-r border-gradient-to-r from-blue-500/30 via-transparent to-transparent relative group pl-6 sm:pl-8">
      ...
      <div className="relative flex items-start text-white space-x-2 sm:space-x-4">
        ...
        <div className="flex flex-col flex-1 min-w-0">
          {/* Arabic Text */}
          <p className="text-lg sm:text-xl font-bold text-right mb-2 leading-relaxed bg-gradient-to-l from-blue-200 to-purple-200 bg-clip-text text-transparent font-arabic">
            {hadith.arabic}
          </p>
          ...
          <p className="text-xs sm:text-sm font-medium text-gray-200 leading-relaxed">
            {hadith.english} 
            <span className="text-[10px] sm:text-xs ml-2 opacity-60 text-gray-400 font-normal">
              ({hadith.source})
            </span>
          </p>
        </div>
...
// --- Main Component ---
export default function HadithTicker() {
  const [hadithData, setHadithData] = useState<Hadith[]>([]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const data: Hadith[] = snapshot.docs.map((doc) => ({
        id: doc.id,
        ...(doc.data() as Omit<Hadith, "id">),
      }));
      setHadithData(data);
    });
  ...
  const infiniteHadith = useMemo(() => {
    if (!hadithCount) return [];
    return [...hadithData, ...hadithData, ...hadithData];
  }, [hadithData]);
  ...
  return (
    <section id="hadith-ticker" className="relative overflow-hidden bg-gradient-to-r from-slate-950 via-slate-900 to-slate-950 py-4 sm:py-6 shadow-2xl border-y border-blue-500/10">
      ...
      {/* Scroll Container */}
      <div className="scroll-container relative flex whitespace-nowrap px-4 sm:px-6">
