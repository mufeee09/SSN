...
const timeToMinutes = (timeInput, prayerName) => {
    ...
    const timeStr =
        typeof timeInput === 'object' && timeInput !== null
            ? timeInput.azan
            : timeInput;
    ...
    // Convert properly to 24h format

    if (['Asr', 'Magrib', 'Isha', 'Sunset'].includes(prayerName)) {
        if (h < 12) h += 12;
    }

    if (['Fajr', 'Sahar End', 'Sunrise'].includes(prayerName)) {
        if (h === 12) h = 0;
    }
...
const getClosestEventDate = (events, currentDay) => {
    for (const event of events) {
        if (event.date.includes('-')) {
            const [startStr, endStr] = event.date.split('-').map(s => s.trim());
            const startDay = parseInt(startStr.match(/\d+/)?.[0] || '1');
            const endDay = parseInt(endStr.match(/\d+/)?.[0] || '31');
...
const TimeBadge = ({ icon: Icon, label, time, isCurrentPrayer = false }) => {
    ...
    return (
        <div
            className={`flex flex-col p-3 rounded-xl transition-all duration-200 shadow-lg ${isCurrentPrayer
                ? 'bg-blue-600/20 border border-blue-400/60'
                : 'bg-slate-700/40 border border-slate-600/40 hover:bg-slate-700/60'
                }`}
        >
            ...
            {hasTwoTimes ? (
                <div className="flex flex-col items-start">
                    <span className={`text-xl font-extrabold font-mono ${isCurrentPrayer ? 'text-blue-50' : 'text-gray-100'
                        }`}>
                        🕌 {time.azan}
                        {isCurrentPrayer && <BellRing size={14} className="inline ml-2 text-blue-400" />}
                    </span>
                    <span className={`text-sm font-semibold font-mono mt-1 ${isCurrentPrayer ? 'text-blue-200' : 'text-gray-400'
                        }`}>
                        ⏰ {time.namaz}
                    </span>
                </div>
            ) : (
                <span className="text-xl font-extrabold font-mono text-gray-200 flex items-center">
...
export default function CalendarSection() {
    ...
    const [allMonthlyEventsRaw, setAllMonthlyEventsRaw] = useState({});
    ...
    useEffect(() => {
        const fetchSchedule = async () => {
            try {
                const snapshot = await getDocs(collection(db, "prayerSchedule"));
                const data = {};

                snapshot.forEach((docSnap) => {
                    data[docSnap.id] = docSnap.data().events || [];
                });

                setAllMonthlyEventsRaw(data);
    ...
    const allMonthlyEvents = useMemo(() => {
        ...
        Object.keys(allMonthlyEventsRaw).forEach(month => {
            processed[month] = allMonthlyEventsRaw[month].map(event => {

                // 🔥 USE FIREBASE TIMES EXACTLY — NO ROUNDING
                const fajrAzan = event.fajr;
                const zuhrAzan = event.zuhr;
                const asrAzan = event.asr_;
                const magribAzan = event.magrib;
                const ishaAzan = event.isha_;
    ...
    const monthNames = [
        ...
        'November',
        'December'
    ].filter(month => allMonthlyEvents[month]);
    const events = allMonthlyEvents[selectedMonth] || [];
    ...
    const activePrayer = useMemo(() => {
        if (!todayEventData) return null;
        return getActivePrayer(todayEventData);
    }, [todayEventData, currentTime]);
    const isTodayEvent = (event) => event === todayEventData;
                        ...
                        {events.map((event) => {
                            ...
                            return (
                                <div key={event.date} className="group relative">
                                    ...
                                    {/* Main Card Container (Glassmorphism Effect) */}
                                    <div className={`relative backdrop-blur-xl rounded-2xl p-6 transition-all duration-300 h-full min-h-[720px] ${isCurrentDayCard
                                        ? 'bg-blue-900/30 border-2 border-blue-500/60 shadow-2xl shadow-blue-900/50' // Highlighted today card
                                        : 'bg-slate-800/50 border border-slate-700/50 hover:border-blue-700/50 hover:bg-slate-700/60 shadow-lg'
                                        }`}>
